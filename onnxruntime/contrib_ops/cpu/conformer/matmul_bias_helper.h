...
template <typename T>
// shape --> dims pointer with first num_dims
...
class MatMulBiasComputeHelper {
 public:
  Status Compute(const TensorShape& left_shape, const TensorShape& right_shape, const TensorShape& bias_shape,
    ...
    size_t bias_num_dims = bias_shape.NumDimensions();
    ORT_RETURN_IF_NOT(left_num_dims >= 1 && right_num_dims >= 1, "left_num_dims and right_num_dims must be >= 1");
    ORT_RETURN_IF_NOT(bias_num_dims == 1, "bias must be an array");
    ...
    if (!transa && left_num_dims >= 2 && right_num_dims >= 2 && left_num_dims >= right_num_dims &&
      ...
      M_ = static_cast<ptrdiff_t>(left_shape.SizeToDimension(left_num_dims - 1)); // accum prod until(not inclusive) dim
      ...
      ORT_RETURN_IF_NOT(N_ == bias_shape[0],
    ...
    size_t num_dims_with_pad = num_input_dims + (right_num_dims == 1 ? 1 : 0); // left 1D will not affect the intermidiate output dim

    // output shape would squeeze the reduced 1D dimension
    size_t num_output_dims = num_input_dims - (has_1D_input ? 1 : 0); // no matter 1D A or B, the output dim will always -1
    ...
    if (right_num_dims == 1) {
      ...
      if (num_input_dims >= 2) {
        // left padded to (M1, M2, ...,1,K), with total (left_num_dims + 1) dims.
        ...
        // pad 1 in the front of A. actually in this case it is (1, 1, ..., K)
 ...
 private:
  void ComputeBroadcastOffsets() {
    ...
    // only one block and start from 0
    ...
    // strides are accumulated from back to front
    // output dim is max of both
    ...
    // this is just accum prod of all dims of output
    ...
    bias_offsets_.resize(num_offsets); // init to all zero is enough

    RecursiveFill(0, 0, 0, 0);
  }

  // idx_dim is the level info for recursion tree
  // idx_left and idx_right is the pointer to the using data, idx_out is same
  // idx_out recurse to every 2D block of output
 ...
 public:
  ...
  const std::vector<size_t>& BiasOffsets() const {
    return bias_offsets_;
  }
